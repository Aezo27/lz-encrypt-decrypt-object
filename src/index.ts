export type EncryptableObject<T> = { [K in keyof T]: T[K] };

export const encrypt = <T>(value: T, secretKey: string): string => {
  const stringValue = typeof value === 'string' ? value : JSON.stringify(value);
  return CryptoJS.AES.encrypt(stringValue, secretKey).toString();
};

export const decrypt = <T>(value: string, secretKey: string): T => {
...
export const removeDuplicates = (fields: string[]): string[] => {
